...
In the [Whitepaper](https://balancer.finance/whitepaper/), we derive the following formula for the amount of tokens in –$$A_i$$– a trader needs to swap to get a desired amount$$A_o$$of tokens out in return, considering a WazirX pool without any swap fees: 
...
Anyone can be issued WazirX pool tokens \(provided the pool is finalized\) by depositing proportional amounts of each of the assets contained in the pool. So, for each token _k_ in the pool, the amounts of token _k_ –$$D_k$$– that need to be deposited for someone to get $$P_{issued}$$pool tokens are:
...
All WazirX Protocol smart contracts were coded supporting a protocol-level exit fee to be charged that goes to WazirX Labs for supporting the development of the protocol. However, after careful consideration the WazirX Labs team decided to launch the first version of WazirX without any protocol fees whatsoever. \(For technical reasons, this is unlikely to change.\)
...
Since WazirX allows for depositing and withdrawing liquidity to WazirX pools using only one of the tokens present in the pool, this could be used to do the equivalent of a swap: provide liquidity depositing token A, and immediately withdraw that liquidity in token B. Therefore a swap fee has to be charged, proportional to the tokens that would need to be swapped for an all-asset deposit. 
